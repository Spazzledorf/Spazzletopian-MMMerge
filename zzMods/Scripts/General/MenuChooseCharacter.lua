...
function events.GameInitialized2()
	...
	local function ClassAvailable(Class, Race, Continent)
		if FreeSelection then
			local Extra = Game.ClassesExtra[Class]
			if not Extra or Extra.Kind == 0 then
				return false
			end
			for k, v in pairs(Game.ClassesExtra) do
				if v.Kind == Extra.Kind and v.Step < Extra.Step then
					return false
				end
			end
			return true
	...
	function events.CharacterChosen()
		MV.PlayersAttrs[0].Race = Game.CharacterPortraits[Party[0].Face].Race
		SwitchClass(0)
	end
	...
	local function SetActive(i, NeedRefresh, IsNew)
		...
		if RosterIdSpots[ActiveChar] then
			...
			MV.PlayersAttrs[RosterIdSpots[ActiveChar]].Race = MV.PlayersAttrs[0].Race
			char_ptr = Party.PlayersArray[RosterIdSpots[ActiveChar]]["?ptr"]
		else
			tmp_race = MV.PlayersAttrs[0].Race
			char_ptr = MO.TmpChar
		end
		...
		if not IsNew then
			if RosterIdSpots[ActiveChar] then
				...
				MV.PlayersAttrs[0].Race = MV.PlayersAttrs[RosterIdSpots[ActiveChar]].Race
			else
				mem.copy(BasePtr, MO.TmpChar, Size)
				MV.PlayersAttrs[0].Race = tmp_race
			end
		end
	...
	local function PrepareChar(RosterId, PartyId)
		MF.LogInfo("%s: PrepareChar: RosterId %d, PartyId %d",
			LogId, RosterId, PartyId)
	...
	local FreeSelectionBtn = CreateButton{
		IconUp = "tab2a",
		IconDown = "tab2b",
		Masked = true,
		Screen = 21,
		...
		Y = 54,
		Action = function(t)
			FreeSelection = not FreeSelection
			t.IUpSrc, t.IDwSrc = t.IDwSrc, t.IUpSrc
			t.IUpPtr, t.IDwPtr = t.IDwPtr, t.IUpPtr

			local ToolTip = "Free class / portrait combinations are allowed now."
			if not FreeSelection then
				ToolTip = "Free class / portrait combinations are disabled now."
			end
			CustomUI.DisplayTooltip(ToolTip, 6, 180, 90, 300)
		end
	}
	...
	function events.MenuAction(t)
		...
		elseif t.Action == 65 then
			...
			for i,v in Game.CharacterPortraits do
				...
				if FreeSelection then
					Available = v.Race < 255
				else
					Available = table.find(RaceByContinent[CurCont], v.Race) ~= nil and not table.find(PortraitsExceptions[CurCont], i)
				end
				v.AvailableAtStart = Available and 1 or 0
			end
		end
	end

	events.AddFirst("NewGameMap", function()
		MF.LogVerbose("%s: NewGameMap", LogId)
		MV.NewGame = true
		--UnloadIcons(21)
