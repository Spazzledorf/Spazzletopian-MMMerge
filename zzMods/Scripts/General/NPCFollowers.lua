...
local function get_casts_num_adj(casts)
	if not casts or not tonumber(casts) then return end
	if casts >= 1 and casts <= 4 then
		return Game.NPCText[2827 + casts]
	else
		return string.format(Game.NPCText[2832], casts)
	end
end
...
local function RebuildFollowersTable()
	...
	MF.LogInfo("%s: gc", LogId)
	collectgarbage("collect")
	MF.LogInfo("%s: gc finished", LogId)
end
...
local function dismiss_npc(NPCId, NoExit)
	...
	if slot then
		...
		if MF.GtSettingNum(MM.HirelingsResetCastsOnDismiss, 0) then
			Game.NPC[NPCId].UsedSpell = 0
		end
		for i = 0, 5 do
	...
	if NPCFollowers.HireState then
		...
		if not slot then
			...
			if mapvars.MapNPCNews and mapvars.MapNPCNews[NPCId] then
				local MonId = mapvars.MapNPCNews[NPCId].Mid
				local Mon = Map.Monsters[MonId]
				Mon.AIState = const.AIState.Invisible

				if Multiplayer then
					Multiplayer.monster_action(MonId, Mon, const.AIState.Invisible)
				end
			else
				for i,v in Map.Monsters do
					if v.NPC_ID == NPCId then
						...
						if Multiplayer then
							Multiplayer.monster_action(i, v, const.AIState.Invisible)
						end
					end
				end
			end
		...
		if ProfSet then
			...
			cText = string.replace(cText, "%18", tostring(ProfSet.Cost))
			cText = string.replace(cText, "%19", get_casts_num_adj(ProfSet.CastsPerDay) or "")
...
local function HireRecruit()
	...
	if npcInfo and Party.count < 5 then
		if NPCFollowers.HireState then
			...
			ExitCurrentScreen()
			Map.Monsters[npcInfo.Mid].AIState = const.AIState.Invisible
			HireCharacter(npcInfo.RecruitId)

			if Multiplayer then
				Multiplayer.monster_action(npcInfo.Mid, Map.Monsters[npcInfo.Mid], const.AIState.Invisible)
			end
		else
			NPCFollowers.HireState = true
...
-- Speak with hired NPC on buttons 6 - 9

local function speak_with_hireling(i)
	if i < 1 or i > 4 then return end
	if vars.HiredNPC[i] then
		evt.SpeakNPC(vars.HiredNPC[i])
	end
end

MF.RegisterExtraKeyBind({DefKey = 0x30, RegValue = "xkey_npcpanel", Label = "NPC PANEL",
	Action = ToggleNPCPanel})

for i = 1, 4 do
	MF.RegisterExtraKeyBind({DefKey = 0x35 + i, RegValue = "xkey_npc" .. i,
		Label = strformat("NPC %d", i),
		Action = function() speak_with_hireling(i) end})
end
...
function events.GameInitialized2()
	...
	function events.LoadMapScripts(WasInGame)
		...
		if not WasInGame then
			...
			Game.NPCTopic[NPCFollowers.HireNPCTopic] = Game.GlobalTxt[406]	-- "Hire"
			Game.NPCTopic[NPCFollowers.HirePeasantEvent] = Game.NPCTopic[100]	-- "Join"
			...
			evt.Global[NPCFollowers.ShowNPCDescrEvent] = function()
				...
				if ProfSet then
					...
					if ProfSet.CastsPerDay then
						cText = cText .. "\n"
							.. string.format(Game.NPCText[2827],	-- "Casts used: %d / %d"
								Game.NPC[npc].UsedSpell, ProfSet.CastsPerDay)
					end
					Message(cText)
				end
			end
...
local function ProcessNPCProf()
	...
	for line in LineIt do
		...
		NPCProf[cnt] = {
			...
			CastsPerDay = tonumber(Words[9]),
			Joins	= Words[10] == "x",
			Recruit	= Words[11] == "x",
			TextFields = {
				Text	= NPCTextId(Words[12]) or Words[12],
				Description = NPCTextId(Words[13]) or Words[13]
				}
			}
