...
mem.autohook2(0x491f58, function(d)
	local player, player_index = GetPlayer(d.eax)
	events.cocall("RegenTick", player, player_index)
end)
...
function events.SpellFailCheck(t)
	if t.Spell == 31 then
		local t1 = {CanCast = true, Handled = false, Mastery = t.Mastery}
		events.call("CanCastTownPortal", t1)
		if t1.Handled then
			t.Chance = 100
		end
		if not t1.CanCast then
			t.Chance = 0
		end
	end
end
...
hook(NewCode + 5, function(d)
	...
	local item = u4[MO.SpellScrollItem]
...
local OverridenSpellSkills = {}
local function RevertOverriden()
	for i, v in pairs(OverridenSpellSkills) do
		Party.PlayersArray[i].Skills[v[1]] = v[2]
	end
	table.clear(OverridenSpellSkills)
	events.Remove("Tick", RevertOverriden)
end
...
mem.autohook(0x42609e, function(d)
	...
	elseif not is_spell_scroll then
		local skill_id = SpellSchoolBySpell(spell_id) + 12
		local value = JoinSkill(t.Skill, t.Mastery)
		if value ~= player.Skills[skill_id] then
			OverridenSpellSkills[player_id] = {skill_id, player.Skills[skill_id]}
			player.Skills[skill_id] = value
			events.Tick = RevertOverriden
		end
	end
end)
]=]
