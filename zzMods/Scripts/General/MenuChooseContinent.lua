...
function events.GameInitialized2()
	...
	local function SlChosen(StartMap, Continent)
		...
		if TownPortalControls.SelectingContinent then
			TownPortalControls.SelectingContinent = false
			if Continent then
				TownPortalControls.SwitchTo(Continent)
				-- Defer the recast to the next L2InterfaceUpd tick rather than
				-- calling CastSpellDirect synchronously from inside this click
				-- handler (itself nested inside CustomUI's click-dispatch, not
				-- a clean top-level frame boundary like a real player-initiated
				-- cast). See AGENTS.md Follow-up 9.
				TownPortalControls.PendingRecast = true
...
function Game.UnlockTPSlots()
	-- Set DefAdrTable slot QBits (game checks these for visibility)
	for _, q in ipairs({181, 180, 184, 183, 182, 185}) do
		Party.QBits[q] = true
	end
	-- Set all continent-specific QBits so SwitchTo copies true values
	-- Jadame (switch 300): QBI 301-306
	-- Antagrich (switch 307): QBI 718-723
	-- Enroth (switch 308): QBI 310-315
	for _, q in ipairs({301,302,303,304,305,306,718,719,720,721,722,723,310,311,312,313,314,315}) do
		Party.QBits[q] = true
	end
	-- Also set the global switch QBits (309 used for continent travel slots)
	Party.QBits[309] = true
	Game.EscMessage("All TP slots unlocked across all continents")
end
