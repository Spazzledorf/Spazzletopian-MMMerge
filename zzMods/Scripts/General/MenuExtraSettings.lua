...
function events.GameInitialized2()
	...
	local ExSetScr4 = NewSettingsPage("MergeExtraSettings4", " Loot & QoL", "ExSetScr2")
	local ExSetScr5 = NewSettingsPage("MergeExtraSettings5", " XP Multiplier", "ExSetScr2")

	local VarsToStore = {"UseMonsterBolster", "BolsterAmount", "ShowWeatherEffects", "ImprovedPathfinding", "CleaveEnabled", "SkillHarmonyEnabled", "PartyLiquidsEnabled", "StatRemixEnabled", "StatTooltipsEnabled", "GuardianEnabled", "ManaShieldEnabled", "RetaliationEnabled", "ItemSystemEnabled", "TownPortalFullList", "StatColorsEnabled", "AutoHealEnabled", "PartyMerchantEnabled", "TrapPerceptionEnabled", "LootAllEnabled", "ProjectileTrackingEnabled", "XpMultiplierEnabled", "XpMultiplier", "XpQuestMultiplierEnabled", "XpQuestMultiplier"}
	...
	local RightSwitch = CustomUI.CreateButton{
		...
		Layer 	= 0,
		Screen 	= {ExSetScr, ExSetScr2, ExSetScr3, ExSetScr4, ExSetScr5},
		DynLoad = true,
		X = 554, Y = 422}
	...
	CustomUI.CreateText{
		...
		X = 215, Y = 175, Width = 385, Height = 16,
		AlignLeft = true,
		Screen = ExSetScr2
	}
	...
	CustomUI.CreateText{
		...
		X = 215, Y = 225, Width = 385, Height = 16,
		AlignLeft = true,
		Screen = ExSetScr2
	}
	...
	CustomUI.CreateText{
		...
		X = 215, Y = 275, Width = 385, Height = 16,
		AlignLeft = true,
		Screen = ExSetScr2
	}
	...
	CustomUI.CreateText{
		...
		X = 215, Y = 325, Width = 385, Height = 16,
		AlignLeft = true,
		Screen = ExSetScr2
	}
	...
	CustomUI.CreateText{
		...
		X = 215, Y = 375, Width = 385, Height = 16,
		AlignLeft = true,
		Screen = ExSetScr2
	}

	-- third page: Combat & Items (5 toggles, 50px spacing)
	...
	CustomUI.CreateText{
		Text = "Continental Teleportation",
		X = 215, Y = 375, Width = 385, Height = 16,
		AlignLeft = true,
		Screen = ExSetScr3
	}

	-- fourth page: Loot & QoL (5 toggles, 50px spacing)
	OnOffTumbler(ExSetScr4, 95, 175, VarsToStore[15])
	CustomUI.CreateText{
	...
	OnOffTumbler(ExSetScr4, 95, 225, VarsToStore[16])
	CustomUI.CreateText{
		Text = "Smart Heal Targeting",
		X = 215, Y = 225, Width = 385, Height = 16,
		AlignLeft = true,
		Screen = ExSetScr4
	}

	OnOffTumbler(ExSetScr4, 95, 275, VarsToStore[17])
	CustomUI.CreateText{
		Text = "Party Merchant (best skill for prices)",
		X = 215, Y = 275, Width = 385, Height = 16,
		AlignLeft = true,
		Screen = ExSetScr4
	}

	OnOffTumbler(ExSetScr4, 95, 325, VarsToStore[18])
	CustomUI.CreateText{
		Text = "Trap Perception (best Perception for traps)",
		X = 215, Y = 325, Width = 385, Height = 16,
		AlignLeft = true,
		Screen = ExSetScr4
	}

	OnOffTumbler(ExSetScr4, 95, 375, VarsToStore[19])
	CustomUI.CreateText{
		Text = "Loot All Corpses (V hotkey)",
		X = 215, Y = 375, Width = 385, Height = 16,
		AlignLeft = true,
		Screen = ExSetScr4
	}

	-- fifth page: XP Multiplier
	OnOffTumbler(ExSetScr5, 95, 175, VarsToStore[21])
	CustomUI.CreateText{
		Text = "XP Multiplier",
		X = 215, Y = 175, Width = 385, Height = 16,
		AlignLeft = true,
	...
	NumberRegulator(ExSetScr5, 103, 205, "XpMultiplier",
		function(t, val)
			Game.XpMultiplier = val
		end,
		function(val)
			return "×" .. tostring(Game.XpMultiplier)
		end,
		Game.XpMultiplier, 1, 10, 1)

	OnOffTumbler(ExSetScr5, 95, 255, VarsToStore[23])
	CustomUI.CreateText{
		Text = "Quest XP Multiplier (risky - huge rewards)",
		X = 215, Y = 255, Width = 385, Height = 16,
		AlignLeft = true,
		Screen = ExSetScr5
	}

	Game.XpQuestMultiplier = Game.XpQuestMultiplier or 2
	NumberRegulator(ExSetScr5, 103, 285, "XpQuestMultiplier",
		function(t, val)
			Game.XpQuestMultiplier = val
		end,
		function(val)
			return "×" .. tostring(Game.XpQuestMultiplier)
		end,
		Game.XpQuestMultiplier, 1, 10, 1)

	OnOffTumbler(ExSetScr5, 95, 345, VarsToStore[20])
	CustomUI.CreateText{
		Text = "Projectile Tracking (homing missiles)",
		X = 215, Y = 345, Width = 385, Height = 16,
		AlignLeft = true,
		Screen = ExSetScr5
	}
	...
	function events.LoadMapScripts(WasInGame)
		if not WasInGame then
			...
			if ExSet.AutoHealEnabled == nil then
				ExSet.AutoHealEnabled = true
			end
			ExSet.XpMultiplier = ExSet.XpMultiplier or 2
			if ExSet.XpMultiplierEnabled == nil then
				ExSet.XpMultiplierEnabled = true
			end
			ExSet.XpQuestMultiplier = ExSet.XpQuestMultiplier or 2
			if ExSet.XpQuestMultiplierEnabled == nil then
				ExSet.XpQuestMultiplierEnabled = false
			end
