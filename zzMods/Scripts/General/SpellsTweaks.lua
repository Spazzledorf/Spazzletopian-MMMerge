...
-- Fix Town Portal SP removal
asmpatch(0x429676, [[
mov word ptr [0x51D81A], ax
cmp dword ptr [ecx + 0x1BFC], eax
]])
asmpatch(0x431280, [[
jl @first
cmp eax, 0x32
...
-- Fix Reanimate "You lack the skill" message
mem.IgnoreProtection(true)
mem.u4[0x42C0CA] = 0x601514
...
function events.PlayerCastSpell(t)
	...
	if Game.CurrentScreen ~= const.Screens.ItemSpell then
		return
	end
	...
	if t.SpellId == 30 then -- Enchant item
		local CurPlayer = Party[Game.CurrentPlayer]
		local item = CurPlayer.Inventory[t.InventorySlot]
		if item <= 0 or item > 138 then
			return
		end

		item = CurPlayer.Items[item]
